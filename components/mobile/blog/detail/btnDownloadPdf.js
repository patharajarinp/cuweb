...
<figure class="image"><img src="http://192.168.6.19:8080/upload/images/1594490302059.jpg"></figure>
...
function toDataURL(url, callback){
  ...
  useEffect(() => {
    ...
    document.querySelectorAll('figure[class="media"]').forEach( (element,index) => {
      if (tmp[index]) {
      element.innerHTML=`<a style="color:gray;text-align: center;" href="${tmp[index].getAttribute( 'url' )}" >${tmp[index].getAttribute( 'url' )}<a/>`;
      }
    });
  ...
  return (
    <div>
        <div className="d-none" id={`pdft${data.id}`}>
                ...
                <img id={`imglogo${data.id}`}  src='/mobile/image/logo.jpg'/>
        ...
        {status == 0 ? (
        <div className="blog-btn-download-pdf-g blog-data-col-2">
        <img src="/mobile/icon/blog/blog-icon-download-w.svg" />
      </div>
      ) : (
    
        <div className="blog-btn-download-pdf blog-data-col-2"onClick={()=>{printPDF()}}>
              <img src="/mobile/icon/blog/blog-icon-download-w.svg" />
            </div>
      )}
