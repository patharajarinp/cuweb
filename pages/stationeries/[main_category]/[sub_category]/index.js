...
const SubProducts = (props) =>{
  const {t, query,products, cates, main_category, sub_category, main, currentLanguage, seller, mainData, subData, cateIndex} = props;
  ...
  const pathname = `/stationeries/${main_category}/${sub_category}`
  ...
  return (
    <Layout cateData={cateIndex}>
      ...
      />
      {
        !isMobile ? (
          <MainFilter 
            t={t} query={query} pathname={pathname} local={local} products={products} type={"stationery"} s_category={s_category}
            setLoading={setLoading} breadcrumb={false} level={2} page_path={`stationeries`} level1_id={main_category}  level2_id={sub_category}
            main={main}
            show_all={false}
            currentLanguage={currentLanguage}
            seller={seller}
          ...
          <>
            ...
            <MobileMainFilter 
              cates={cates}
              t={t} query={query} pathname={pathname} products={products} type={"stationery"} s_category={s_category}
              setLoading={setLoading} breadcrumb={false} level={2} page_path={`stationeries`} level1_id={main_category}  level2_id={sub_category}
              main={main}
              currentLanguage={currentLanguage}
              isSeller={seller ? true : false}
...
SubProducts.getInitialProps = async ({query, req}) => {
  const { main_category, sub_category, shop_name } = query
  ...
  try {
    ...
    var params = { ...query, limit: LIMIT, type : 'non_book', search : query.text };
    ...
    const [product_res,cate_res,care_data] = await Promise.all([
      ...
      api.getCateByType({main_category, sub_category}),
      api.getCate()
    ])
    ...
    return {query,products,cates, cateIndex, main_category : main.url_name, sub_category : sub.url_name, main : sub,currentLanguage, seller, 
      mainData : main, subData : sub}
  }
  catch (err) {
    console.log(err)
    throw err;
...
export default withTranslation(['header', 'product', 'filter', 'mobile_header', 'mobile_products', 'mobile_filter']) (SubProducts)