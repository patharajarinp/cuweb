      ...
      <div className="container">
        ...
        <div className="row my-5">
          <div className="col-12">
            ...
            <h1 className="seo-text">{`หมวดคอร์สออนไลน์`}</h1>
            {/* <h2>หมวดคอร์สออนไลน์</h2> */}
              {/* <h2>{t('ebook_category')}</h2> */}
            </div>
          </div>
        </div>
        <div className="row">
          <div className="col-12 main-course">
          <Slick {...settings}>
            {
                  ...
                  <Link href={`/courses/[main_category]?main_category=${val.url_name}`} as={`/courses/${val.url_name}`}>
                    <a className="text-default">
                      <div className="btn-cate">
                        <img src={val.image_ebook ? val.image_ebook: `${api.frontend_url}/icon/mock-course-icon.svg`} />
                      </div>
                      <div className="text">
                ...
                (val.products && val.products.length == 4) && (
                  <div className="float-right">
                    <Link href={`/recommend/[key]?key=${val.key}`} as={`/recommend/${val.key}`}>
                      <a>
                        <button className="btn btn-outline-course ">{t('view_all')}</button>
                      </a>
                    </Link>
                  </div>
                )
