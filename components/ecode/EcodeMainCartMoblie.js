...
const EcodeMainCartMoblie = (props) => {
  const { t } = props;
  ...
  useEffect(() => {
    if (!user || !cartDetail || !shippingInfo) return;
    if (!user.cart.length) return;
  
    // ✅ ถ้ายังไม่มี address ไม่ต้องคำนวณ shipping แต่คำนวณราคาสินค้าล้วน ๆ แทน
    if (!cartDetail.address_id || !shippingInfoCu) {
      const total = user.cart.reduce((sum, item) => sum + item.cover_price * item.quantity, 0);
      setCalDetail({
        total_discount: total,
        shipping: 0,
        total_price_cu: total
      });
      return;
    }
  
    // ✅ ถ้ามี address แล้ว คำนวณปกติ
    tools
          ...
          {!check.length ? (
            <a className="btn-cart-to-payment">
              <h4 className="text-white m-auto">
                {t("mobile_shippingInfo:select_products")}
              </h4>
            </a>
          ) : (
...
export default EcodeMainCartMoblie