...
const SelectFilter = ({ t, setOpen, open, reset, type, main_category, sub_category, s_category, category, 
  CateShow, size, only_seller, resetAll,show_all }) => {
      ...
      size == "md" ? (
        ...
        <Collapse in={open} className="col-12">
          <div id="collapse-fill" className="collapse-fill">
            <div className="fill-onscroll">
              {
                only_seller && 
                <p className="px-4 d-flex tags">
                  <i onClick={resetAll} className="fa fa-times mr-2 box"></i><span>{t('product:filter_marketplace')}</span>
                </p>
              }
              {
                type && 
                <p className="px-4 d-flex tags">
                  <i onClick={resetAll} className="fa fa-times mr-2 box"></i><span>{t('product:type_product')} : {t(type)}</span>
                </p>
              }
              {
        ...
        <div className="py-4 custom-filter">
          ...
          }
          {
            type && 
            <p className="px-4 d-flex tags">
              <i onClick={resetAll} className="fa fa-times mr-2 box"></i><span>{t('product:type_product')} : {t("filter:"+type)}</span>
            </p>
          }
          {
            main_category && s_category && 
