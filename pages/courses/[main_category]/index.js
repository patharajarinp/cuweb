...
const MainProducts = (props) =>{
  ...
  const pathname = `/courses/${main_category}`
      ...
      />
      {
        !isMobile ? (
          <MainFilter 
            ...
            setLoading={setLoading} breadcrumb={false} level={1} page_path={`courses`} level1_id={main_category} 
            main={main}
            show_all={false}
            currentLanguage={currentLanguage}
            seller={seller}
...
MainProducts.getInitialProps = async ({query, req}) => {
  ...
  try {
    ...
    if(main) {
      var params = { ...query, limit: LIMIT, type : 'course', search : query.text, main_category : main.id };
