...
const MainProducts = (props) =>{
  ...
  const pathname = `/books/${main_category}`
...

      {
        !isMobile ? (
          <MainFilter 
              ...
              setLoading={setLoading} breadcrumb={false} level={1} page_path={`books`} level1_id={main_category} 
...
MainProducts.getInitialProps = async ({query, req}) => {
  ...
  try {
    ...
    if(main) {
      var params = { ...query, limit: LIMIT, type : 'book', search : query.text, main_category : main.id };
