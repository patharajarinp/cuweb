...
const MainProducts = (props) =>{
  const {t, query,products, cates, main_category, main, currentLanguage, seller, mainData, cateIndex} = props;
  ...
  const pathname = `/stationeries/${main_category}`
      ...
      />
      {
        !isMobile ? (
          <MainFilter 
            ...
            setLoading={setLoading} breadcrumb={false} level={1} page_path={`stationeries`} level1_id={main_category} 
            main={main}
            show_all={false}
            currentLanguage={currentLanguage}
            seller={seller}
...
MainProducts.getInitialProps = async ({query, req}) => {
  const { main_category, shop_name } = query
  ...
  try {
    ...
    if(main) {
      var params = { ...query, limit: LIMIT, type : 'non_book', search : query.text, main_category : main.id };
      delete params.sub_category
      delete params.category
      // console.log('main', main);
      const [product_res,cate_res,care_data] = await Promise.all([
        api.getProducts(params),
        api.getCateByType({main_category : main.id}),
        api.getCate()
      ])
      ...
      return {query,products,cates, cateIndex, main_category : main.url_name, main,currentLanguage, seller, mainData : main}
    }
  }
  catch (err) {
    console.log(err)
