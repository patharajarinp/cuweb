...
const SummaryFirstStep = ({ t, user, setUser, calDetail, promotion, setPromotion, setCalDetail, setCartDetail, cartDetail, setStep, pointState, setPointState, setCalDetailTemp }) => {
  ...
  const getUsePoint = () => {
    ...
    let _mod = parseInt(POINT_BALANCE / 20),
      mod = parseInt(point / 20)
    var inputPoint = document.getElementById("point");

    if (mod <= _mod)
      inputPoint.value = mod * 20;
    else {
      alert(`${t('not_enough_points')}\n${t('you_have_points')} : ` + POINT_BALANCE)
      inputPoint.value = (_mod * 20)
    }
          ...
          .then(res => {
            ...
            if (!pass.seller) {
              ...
              data.forEach((dt, index) => {
                pass.seller[dt.id] = {};
                pass.seller[dt.id].shipping_type = 1
                if (dt.id != 'cu') {
  ...
  //  useEffect(()=>{
  //   cartDetail && tools.calAll(user,selected_address,cartDetail.shipping_type,user && user.member && user.member.STATUS == "Y" && user.member.MEMBER_STAT == "N" ,cartDetail.cart_id).then(data=>{
  //     setCalDetail(data)
  //   })
  // },[pointState])
        ...
        <tr key={val.id}>
          ...
          <td>
            ...
            <div className="d-flex align-items-center">
              ...
              })}>{(val.type == 'book') ? t('header:book_menu') : val.type == 'non_book' ? t('header:stationary') : val.type == 'ebook' ? t('header:e_book') : val.type == 'course' ? t('header:course_online') : val.type == 'course_ecode' && t('header:course_online_ecode')}</div>
  ...
  const calNetTotal = () => {
    ...
    let member_discount = (parseInt(pointState) / 2)
                          ...
                          <div className="sum mt-3">
                            ...
                            {pointState > 0 ? (
                              <>
                                ...
                                <div className="sum-price d-flex justify-content-between">
                                  <h5>{t('label_point')}</h5>
                                  <p className="text-num">- {tools.currencyFormatDE(pointState / 2)}</p>
                                </div>
                                <hr />
                              </>
                            ) : ''
                            ...
                            {require_tax == 1 && !taxAddress ? <div className="text-danger my-3">{t('tax_required')}</div> : null}
                            {!canCodShip() ? <div className="text-danger my-3">{t('cod_cant_ship')}</div> : null}
                            <div className="mt-3">
                              {
                                user ? (
                                  (canCodShip() && (require_tax == 1 && taxAddress)) || (canCodShip() && require_tax == 0) ? (
