...
import Seller from '../../../components/cart/SellerCart';
...
const MainCart = (props) => {
  ...
  useEffect(() => {
    ...
    cartDetail.seller && Object.keys(cartDetail.seller).forEach((s) => {
      if (seller_cart[s]) {
        ...
        // let isAllEbook = true;
        let ship = filtered_cart.findIndex(val => val.type != 'ebook' && val.type != 'course' && val.type != 'course_ecode') != -1;
  ...
  const canShipping = () => {
    ...
    console.log(calDetail, 'canShipping')
  ...
  const canCodShip = () => {
    ...
    // console.log(calDetail, 'canCodShip')
    var cod_count = 0;
                  ...
                  <div className="bg-white br-8 p-3">
                    ...
                    <div className="sum mt-3">
                        ...
                        calDetail.discount_shelf ? calDetail.discount_shelf.total_discount > 0 &&
                          <>
                            ...
                            <div className="sum-price d-flex justify-content-between">
                              ...
                              <p className="text-num">฿ {currencyFormatDE(calDetail.total_discount)}test</p>
                            </div>
                            <hr />
                          </> : ''
                      }
                      ...
                      <div className="mt-3">
                        {
                          user ? (
                            user.addresses.length > 0 && cartDetail.cart_id.length > 0 && canShipping() && canCodShip() && haveDisable == false ? (
