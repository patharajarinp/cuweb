...
const EcodeMainSummaryMobile = (props) => {
...
/*
  ...
  */
  ...
  const checkTaxAddress = () => {
    let check = require_tax;
    if (check == 1 && user && user.addresses.length > 0) {
      if (user.addresses[selectedTax] && user.addresses[selectedTax].firstname) {
        return true;
      } else {
        return false;
      }
            ...
            {/*
              user && user.addresses.length > 0 ?
                <>
                  ...
                  {require_tax == 1 && !checkTaxAddress() && <p style={{ fontSize: "12px", color: "red" }}>{t('mobile_summary:tax_required')}</p>}
                </> :
...

                {
                  pointState > 0 ? (
                    <div className="d-flex justify-content-between mt-3">
                      ...
                      <h6 className="font-weight-bold mb-0">- {tools.currencyFormatDE(pointState / 2)}</h6>
                    </div>
                  ) : ''
                }
...
export default EcodeMainSummaryMobile