...
const SubProducts = (props) =>{
  ...
  const pathname = `/courses/${main_category}/${sub_category}`
      ...
      />
      {
        !isMobile ? (
          <MainFilter 
            ...
            setLoading={setLoading} breadcrumb={false} level={2} page_path={`courses`} level1_id={main_category}  level2_id={sub_category}
            main={main}
            show_all={false}
            currentLanguage={currentLanguage}
            seller={seller}
...
SubProducts.getInitialProps = async ({query, req}) => {
  ...
  try {
    ...
    var params = { ...query, limit: LIMIT, type : 'course', search : query.text };
