...
const MoreDetail = ({data, btnName, selectModal}) => {
      ...
      <div className="row my-4">
        ...
        }
        {
          (data.cancel_note && data.status == '02') && (
            <p className="mb-0"><span className="p-medium">ข้อพิพาท ร้านค้าไม่รับคืนสินค้า : </span> <span>{data.cancel_note}</span></p>
          )
        }
        {
          (data.cancel_note && data.status == '01') && (
            <p className="mb-0"><span className="p-medium">ร้านค้าไม่รับคืนสินค้า : </span> <span>{data.cancel_note}</span></p>
          )
        }
        {
          (data.change_type_note) && (
            <p className="mb-0"><span className="p-medium">ข้อมูลเพิ่มเติม ไม่รับคืนให้เปลี่ยนสินค้า : </span> <span>{data.change_type_note}</span></p>
          )
        }
        {
          (data.dispute2_note) && (
