...
import SummaryFirstStep from './SummaryFirstStep';
...
const EcodeMainSummaryDesktop = (props) => {
  ...
  const testemail = () => {
    api.testbank({ 'key': cartDetail.order_id })
    // console.log(cartDetail.order_id);
  }
  ...
  const saveSlip = () => {
    ...
    api.uploadSlip(data)
      .then(res => {
        ...
        const data = res.data;
        setShow3(false);
        Router.push(`/ecode/success_order/[order_id]?order_id=${cartDetail.order_id}`, `/ecode/success_order/${cartDetail.order_id}`);
      })
      .catch(err => {
        setLoadButton(false)
        console.log(err.response);
  ...
  const createShopeePayFunc = () => {
    ...
    data.site = "pc"
    data.return_url = `${api.frontend_url}/ecode/success_order/${cartDetail.order?.order_id}`
            ...
            <div className="col-xl-8 col-12">
              {
                ...
                <div className="row mx-0">
                  ...
                  {/* <div>
                <button className="btn btn-primary" onClick={testemail}>testbank</button>
                </div> */}
                </div>
              }
            </div>
...
export default EcodeMainSummaryDesktop