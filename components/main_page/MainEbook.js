        ...
        <div className="row my-5">
          <div className="col-12">
            <div className="text-center">
              <h1 className="seo-text">{t('ebook_category')}</h1>
              {/* <h2>{t('ebook_category')}</h2> */}
            </div>
          </div>
        </div>
        <div className="row">
          <div className="col-12 main-ebook">
          <Slick {...settings}>
            {
              cate ? cate.map((val, index) => (
                <div className="cate">
                  <Link href={`/ebooks/[main_category]?main_category=${val.url_name}`} as={`/ebooks/${val.url_name}`}>
                    <a className="text-default">
                      <div className="btn-cate">
                        <img src={val.image_ebook ? val.image_ebook: `${api.frontend_url}/icon/mock-icon.svg`} />
                      </div>
                      <div className="text">
                ...
                (val.products && val.products.length == 4) && (
                  <div className="float-right">
                    <Link href={`/recommend/[key]?key=${val.key}`} as={`/recommend/${val.key}`}>
                      <a>
                        <button className="btn btn-outline-ebook ">{t('view_all')}</button>
                      </a>
                    </Link>
                  </div>
                )
