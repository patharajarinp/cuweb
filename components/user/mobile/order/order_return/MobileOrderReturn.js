...
import Returnlist from '../../../../../components/mobile/change/return-list';
import NoOrder from '../../../../../components/mobile/NoOrder';
import api from '../../../../../utils/api';
import AuthService from '../../../../../utils/AuthService';
import { Link } from '../../../../../utils/i18n';

const MobileOrderReturn = (props) => {
  const { t } = props;
    ...
    <>
      <div className="cart-nav">
        <div className=" text-center cart-nav-title">
          <h4 className="text-black">{t("mobile_translations:return_products")}</h4>
        </div>
        ...
        >
          {
            order ? order.rows.map((val, index) => (
              <Returnlist  t={t} order={val} key={val.order_id} />
            )) : null
          }
        </InfiniteScroll>
        {
...
export default MobileOrderReturn