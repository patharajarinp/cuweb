...
const MainProducts = (props) =>{
  const {t, query,products, cates, seller, currentLanguage, cateIndex} = props;
  ...
  return (
    <Layout isFooter={false} active={2}  cateData={cateIndex}>
      {loading && <Loading />}
      <CateMetaTag 
      ...
      />
      {
        !isMobile ? (
          <MainFilter 
            t={t} query={query} pathname={pathname} local={local} products={products} type={"categories"} s_category={s_category}
            setLoading={setLoading} breadcrumb={false} level={0} page_path={`categories`} 
            show_all={true}
            seller={seller}
...
MainProducts.getInitialProps = async ({query, req}) => {
  ...
  try {
    var params = { ...query, limit: LIMIT, search : query.text };
    ...
    return {query,products,cates,seller,currentLanguage, cateIndex}
  }
  catch (err) {
    console.log(err)
    throw err;
